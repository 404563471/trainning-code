...
#/usr/local/lib/R/site-library/simpleSingleCell/doc
library("RNAseq123")
```
...
```{r}
x <- read.table("data/data/expr.txt", header=T, sep="\t", row.name=1)

df <- x[apply(x,1,sum)>0,]
...
write.table(topFDR,"output.xls",sep="\t",quote=F,row.names=F)

detags<-rownames(topFDR)
plotSmear(de, de.tags=detags, main="FC plot")
...
library(biomaRt)
ensembl <- useMart("ENSEMBL_MART_ENSEMBL",host = "jul2015.archive.ensembl.org")
mart <- useMart("ENSEMBL_MART_ENSEMBL", "hsapiens_gene_ensembl",host = "jul2015.archive.ensembl.org")
#biomart=getBM(attributes=c("entrezgene","hgnc_symbol","ensembl_gene_id", "ensembl_transcript_id","go_id","wikigene_id"),mart = mart)
biomart=getBM(attributes=c("entrezgene","go_id"),mart = mart)

gene2goInfo=biomart[biomart$go_id != "", ]
geneID2GO = by(gene2goInfo$go_id, gene2goInfo$entrezgene, function(x) as.character(x))
geneNames <- names(geneID2GO)

geneTable <- read.table(file="output.xls",sep="\t",header=T)
GOI <- unique(geneTable$GID)
...
resultCFis <- runTest(BPdata, algorithm = "classic", statistic = "fisher")
gtFis <- GenTable(BPdata, classicFisher = resultCFis, orderBy = "classic", ranksOf = "classicFisher", topNodes = 20)

goID <- gtFis[1,"GO.ID"]
GOgenes <- genesInTerm(BPdata,goID)[[1]]
sigGOI <- intersect(GOI,GOgenes)

goID <- gtFis[1:5,"GO.ID"]
GOgenes <- genesInTerm(BPdata,goID)
sigGOI = sapply(names(GOgenes),function(x){intersect(GOgenes[[x]],GOI)})

```

## Shared molecular neuropathology across major psychiatric disorders parallels polygenic overlap
=================================================================
https://github.com/mgandal/Shared-molecular-neuropathology-across-major-psychiatric-disorders-parallels-polygenic-overlap
```{r}
